<template>
    ...
    <div class="home">
      <table>
        ...
        <tbody>
          <tr v-for="(rezerwacja, index) in rezerwacjeSlubow" :key="index">
            ...
            <td data-label="Informacje">
              <router-link :to="'/informacje-rezerwacje-slubow/' + rezerwacja.id">Więcej</router-link>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  fetchRezerwacjeSlubow() {
      axios.get('http://46.41.148.115/api/rezerwacje-slubow-dane') // Tu podaj ścieżkę do API w Laravel
        .then(response => {
          this.rezerwacjeSlubow = response.data; // Zaktualizuj dane w komponencie
        })
        .catch(error => {
