<template>
  ...
  <div class="home">
    <table>
      ...
      <tbody>
        <tr v-for="(rezerwacja, index) in rezerwacjeBiznesowe" :key="index">
          ...
          <td data-label="Informacje">
            <router-link :to="'/informacje-rezerwacje-biznesowe/' + rezerwacja.id">Więcej</router-link>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  if (localStorage.getItem('isLoggedIn') === 'true') {
    ...
    if (this.userRole === 'admin') {
      ...
      router.push('/lista-rezerwacje-biznesowe');
    } else {
...
fetchRezerwacjeBiznesowe() {
    axios.get('http://46.41.148.115/api/rezerwacje-biznesowe-dane') // Tu podaj ścieżkę do API w Laravel
      .then(response => {
        this.rezerwacjeBiznesowe = response.data; // Zaktualizuj dane w komponencie
      })
      .catch(error => {
